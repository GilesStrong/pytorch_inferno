...
__all__ = ['VariableSoftmax', 'AbsInferno', 'AbsInferno', 'PaperInferno', 'InfernoPred']
...
class AbsInferno(AbsCallback):
    ...
    def on_train_begin(self) -> None:
        ...
        for c in self.wrapper.cbs:
            if hasattr(c, 'loss_is_meaned'): c.loss_is_meaned = False  # Ensure that average losses are correct
        self.alpha = torch.zeros((self.n_alphas+2 if self.float_b else self.n_alphas+1), requires_grad=True, device=self.wrapper.device)  # Nuisances set to zero (true values)
        with torch.no_grad(): self.alpha[0] = self.true_mu  # POI set to true value
