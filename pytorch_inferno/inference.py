...
__all__ = ['bin_preds', 'get_shape', 'get_syst_shapes', 'get_syst_shapes_as_tensor', 'likelihood_from_scan',
...
# Cell
def get_syst_shapes(bkg_data:np.ndarray, df:pd.DataFrame, model:ModelWrapper,
...
# Cell
def get_syst_shapes_as_tensor(bkg_data:np.ndarray, df:pd.DataFrame, model:ModelWrapper, r_vals:List[float]=[0], l_vals:List[float]=[3]) -> Tensor:
    shapes = []
...
def get_likelihood_width(nll:np.ndarray, val:float=0.5, mu_scan:np.ndarray=np.linspace(20,80,61)) -> float:
    r = InterpolatedUnivariateSpline(mu_scan, nll-val).roots()
    if len(r) == 0: raise ValueError(f'No roots found at {val}, set val to a larger value.')
    return (r[1]-r[0])/2

# Cell
def interp_shape(alpha, c_nom, c_up, c_dw):
    alpha_t = torch.repeat_interleave(alpha.unsqueeze(-1), repeats=c_nom.shape[0], dim=-1)
    a = 0.5*(c_up + c_dw)-c_nom
    b = 0.5*(c_up - c_dw)-c_nom

    switch = torch.where(alpha_t < 0., c_dw - c_nom, c_up - c_nom)
    ...
    return c_nom + torch.sum(abs_var, axis=1)
...
def likelihood_from_updw(f_s:Tensor, f_b_nom:Tensor, f_b_up:Tensor, f_b_dw:Tensor, n:int=1050,
    ...
    opt = partialler(calc_profile_nll, s_true=true_mu, b_true=n-true_mu, f_s=f_s,
